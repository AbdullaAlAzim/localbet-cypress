...
export class profilePage {
  profileLocators = {
    ProfileMenu:
      "//div[@class='flex items-center gap-3 bg-[#F4F4F4] rounded px-2 py-1 relative cursor-pointer']",
    ...
    document_filed_click:
      "//span[@class='ant-select-selection-item' and @title='--']",
    national_id: "//div[@class='ant-space-item']//p[text()='National Id']",
    input_filed:
      "//div[@class='flex items-center justify-between content-info-right']//input[@placeholder='Enter Document ID']",
    upload_button:
      "//button[@type='button' and contains(@class, 'upload-btn') and .//span[text()='Upload Files']]",
    contact_info_m:
      "//div[contains(@class, 'bg-[#dee3ff]')]/img[@alt='checked' and contains(@src, 'edit-icon.svg')]",
    contact_info_m_b:
      "//div[contains(@class, 'ant-space-item')]//button[contains(@class, 'confirm-yes-btn') and span[text()='Yes ']]",
  };
  ...
  //birthday
  birthdaySelect() {
    cy.get(this.profileLocators.birthDay, { timeout: 10000 })
      .should("be.visible")
      .then((input) => {
        cy.wrap(input).click();
        cy.wrap(input).clear();
  ...
  // gender select
  genderSlect() {
    cy.xpath("(//div[@class='ant-select-selector'])[2]").click({
      multiple: true,
      force: true,
    });
    cy.wait(1000);
    cy.xpath(
      "//div[@class='w-full flex items-center gap-1']/p[text()='Male']"
    ).click({ multiple: true, force: true });
  }

  //State feature
  StateSlect() {
    cy.xpath("(//div[@class='ant-select-selector'])[3]").click({
      multiple: true,
      force: true,
    });
    cy.xpath("(//p[contains(text(),'Rangpur Division')])[1]").click({
      multiple: true,
      force: true,
    });
  }
  ...
  //city feature
  citySlect() {
    cy.xpath("(//p[contains(text(),'Chilmāri')])[1]").click({
      multiple: true,
      force: true,
    });
    cy.xpath("(//p[contains(text(),'Chilmāri')])[1]")
      .should("be.visible")
      .click({ multiple: true, force: true });
  }
  ...
  Verify_idendity() {
    ...
    cy.get(this.profileLocators.verify_button, { timeout: 5000 }).click({
      force: true,
    });
    cy.get(this.profileLocators.verify_button, { timeout: 5000 })
      .should("be.visible")
      .click({ force: true });
    cy.get(this.profileLocators.verify_button, { timeout: 5000 })
      .should("exist")
  ...
  //document type
  Document_type() {
    cy.xpath(this.profileLocators.document_filed_click).click({
      multiple: true,
      force: true,
    });
  ...
  Image_upload = () => {
    // Attach files
    cy.xpath(
      "(//span[@class='underline cursor-pointer' and text()='Browse'])[1]"
    ).click();
    ...
    cy.xpath(
      "(//span[@class='underline cursor-pointer' and text()='Browse'])[2]"
    ).click();
    cy.get('input[type="file"]').eq(2).attachFile("user.jpg");

    //cy.xpath("//button[contains(@class, 'upload-btn')]").should('be.visible').click();
    cy.xpath(
      "//button[@class='ant-btn css-1c0na6j ant-btn-primary ant-btn-lg flex items-center justify-center upload-btn gap-1']"
    )
      .should("be.visible")
      .click();
  };
  ...
  phone_add() {
    cy.xpath(
      "(//div[@class='bg-[#dee3ff] sm:w-[34px] w-[25px] sm:h-[32px] h-[25px] flex justify-center items-center rounded-sm cursor-pointer'])[1]"
    ).click({ multiple: true, force: true });
    ...
    function enterOtp() {
      ...
      otp.split("").forEach((digit, index) => {
        cy.get(
          `input[aria-label="Please enter OTP character ${index + 1}"]`
  ...
  profile_email_add() {
    cy.xpath(
      "//div[@class='bg-[#dee3ff] sm:w-[34px] w-[25px] sm:h-[32px] h-[25px] flex justify-center items-center rounded-sm cursor-pointer']"
    ).click({ multiple: true, force: true });
  ...
  profile_email_edit() {
    cy.xpath(
      "(//div[@class='bg-[#dee3ff] sm:w-[34px] w-[25px] sm:h-[32px] h-[25px] flex justify-center items-center rounded-sm cursor-pointer'])[2]"
    ).click({ multiple: true, force: true });
